...
export default function DashboardPage() {
  ...
  const fetchUsers = async () => {
    ...
    setLoading(true);
    try {
      const users = await getAllVerifiedUsers(false, currentUser.email);
      setAllUsers(users);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white p-6">
      {/* Header */}
      <h1 className="text-[28px] font-semibold text-[#212121] mb-4">
        Dashboard
      </h1>
            ...
            {filteredUsers.map((user) => (
              <div key={user.id} className="py-2">
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[12px] font-semibold text-[#212121]">{user.name}</span>
                </div>
